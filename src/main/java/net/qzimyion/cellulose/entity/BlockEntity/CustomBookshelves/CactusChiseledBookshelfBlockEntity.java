...
import net.qzimyion.cellulose.blocks.custom_blocks.ChisledBookshelvesStuff.CactusChiseledBookshelfBlock;
...
public class CactusChiseledBookshelfBlockEntity extends BlockEntity implements Inventory {
    private static final Logger LOGGER = LogUtils.getLogger();
    private final DefaultedList<ItemStack> inventory = DefaultedList.ofSize(3, ItemStack.EMPTY);
    private int lastInteractedSlot = -1;
    public CactusChiseledBookshelfBlockEntity(BlockPos pos, BlockState state) {
        super(CelluloseEntities.CACTUS_CHISELED_BOOKSHELF, pos, state);
    }

    private void updateState(int interactedSlot){
        if (interactedSlot < 0 || interactedSlot >= 3) {
            LOGGER.error("Expected slot 0-2, got {}", interactedSlot);
            return;
        }
        this.lastInteractedSlot = interactedSlot;
        BlockState blockState = this.getCachedState();
        for (int i = 0; i < CactusChiseledBookshelfBlock.SLOT_OCCUPIED_PROPERTIES.size(); ++i) {
            boolean bl = !this.getStack(i).isEmpty();
            BooleanProperty booleanProperty = CactusChiseledBookshelfBlock.SLOT_OCCUPIED_PROPERTIES.get(i);
