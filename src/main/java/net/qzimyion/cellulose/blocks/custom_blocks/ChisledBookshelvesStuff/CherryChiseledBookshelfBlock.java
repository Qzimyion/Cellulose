...
import net.qzimyion.cellulose.entity.BlockEntity.CustomBookshelves.CherryChiseledBookshelfBlockEntity;
...
public class CherryChiseledBookshelfBlock extends BlockWithEntity {
    ...
    public CherryChiseledBookshelfBlock(Settings settings) {
        super(settings);
    ...
    public ActionResult onUse(BlockState state, World world, BlockPos pos, PlayerEntity player, Hand hand, BlockHitResult hit) {
        ...
        if (!(blockEntity instanceof CherryChiseledBookshelfBlockEntity rotatedIrregularFiveSlotChiseledBookshelfBlockEntity)) {
            return ActionResult.PASS;
        }
        Optional<Vec2f> optional = CherryChiseledBookshelfBlock.getHitPos(hit, state.get(HorizontalFacingBlock.FACING));
        if (optional.isEmpty()) {
            return ActionResult.PASS;
        }
        int i = CherryChiseledBookshelfBlock.getSlotForHitPos(optional.get());
        if (state.get(SLOT_OCCUPIED_PROPERTIES.get(i))) {
            CherryChiseledBookshelfBlock.tryRemoveBook(world, pos, player, rotatedIrregularFiveSlotChiseledBookshelfBlockEntity, i);
            return ActionResult.success(world.isClient);
        }
        ItemStack itemStack = player.getStackInHand(hand);
        if (itemStack.isIn(ItemTags.BOOKSHELF_BOOKS)) {
            CherryChiseledBookshelfBlock.tryAddBook(world, pos, player, rotatedIrregularFiveSlotChiseledBookshelfBlockEntity, itemStack, i);
            return ActionResult.success(world.isClient);
        }
    ...
    private static void tryAddBook(World world, BlockPos pos, PlayerEntity player, CherryChiseledBookshelfBlockEntity blockEntity, ItemStack stack, int slot){
        if (world.isClient) {
            return;
        }
    ...
    private static void tryRemoveBook(World world, BlockPos pos, PlayerEntity player, CherryChiseledBookshelfBlockEntity blockEntity, int slot){
        if (world.isClient) {
            return;
        }
    ...
    public void onStateReplaced(BlockState state, World world, BlockPos pos, BlockState newState, boolean moved) {
        CherryChiseledBookshelfBlockEntity irregularFiveSlotBookshelfBlockEntity;
        if (state.isOf(newState.getBlock())) {
            return;
        }
        BlockEntity blockEntity = world.getBlockEntity(pos);
        if (blockEntity instanceof CherryChiseledBookshelfBlockEntity && !(irregularFiveSlotBookshelfBlockEntity = (CherryChiseledBookshelfBlockEntity) blockEntity).isEmpty()){
            for (int i = 0; i < 4; ++i) {
    ...
    public int getComparatorOutput(BlockState state, World world, BlockPos pos){
        ...
        BlockEntity blockEntity = world.getBlockEntity(pos);
        if (blockEntity instanceof CherryChiseledBookshelfBlockEntity irregularFiveSlotBookshelfBlockEntity){
    ...
    public BlockEntity createBlockEntity(BlockPos pos, BlockState state) {
        return new CherryChiseledBookshelfBlockEntity(pos, state);
    }
}
