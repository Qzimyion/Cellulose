...
public class BambooLeafBlock extends Block {
    ...
    public BambooLeafBlock(Settings settings) {
        super(settings);
    }
    ...
    public BlockState getPlacementState(ItemPlacementContext ctx) {
        ...
        for (Direction direction : directions = ctx.getPlacementDirections()) {
            if (!direction.getAxis().isHorizontal() || !(blockState = Objects.requireNonNull(blockState).with(FACING, direction.getOpposite())).canPlaceAt(worldView, blockPos)) continue;
            return blockState;
        }
        return null;
    }
